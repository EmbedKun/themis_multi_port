...
} elseif {[info exists ::env(HESTIA_U200_TOP_DDR_STRESS_LIGHT_BUILD_ROOT)] && $::env(HESTIA_U200_TOP_DDR_STRESS_LIGHT_BUILD_ROOT) ne ""} {
  set build_root [file normalize $::env(HESTIA_U200_TOP_DDR_STRESS_LIGHT_BUILD_ROOT)]
} else {
  set build_root [file join $repo_dir build hestia_u200_top_ddr_stress_light_sim]
}
...
set stress_batch_slots [themis_env_or HESTIA_STRESS_BATCH_SLOTS 32]
set stress_port_queue_depth [themis_env_or HESTIA_STRESS_PORT_QUEUE_DEPTH 64]
...
set stress_drain_start_packets [themis_env_or HESTIA_STRESS_DRAIN_START_PACKETS 128]
set stress_drain_period_cycles [themis_env_or HESTIA_STRESS_DRAIN_PERIOD_CYCLES 1]
...
set xelab_cmd [list xelab --debug off --relax --mt 8 -L work -L unisims_ver -L unimacro_ver -L secureip -s tb_hestia_u200_top_ddr_stress_light_behav work.tb_hestia_u200_top_ddr_stress work.glbl]
...
set xsim_cmd [list xsim tb_hestia_u200_top_ddr_stress_light_behav -wdb /dev/null -tclbatch [file join $build_root xsim_run.tcl] -log simulate.log]
...
if {[string first "PASS: Hestia U200 DDR stress sustained $stress_max_packets packets" $sim_text] < 0 ||
  ...
  puts "ERROR: U200 DDR light stress simulation did not reach a clean PASS marker"
  cd $old_dir
  exit 1
}
