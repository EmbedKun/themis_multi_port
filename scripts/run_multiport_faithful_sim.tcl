...
if {[llength $argv] > 0} {
  ...
  set build_root [file join $repo_dir build hestia_faithful_sim]
}
...
set sim_files [list [file join $repo_dir sim tb_hestia_faithful.sv]]
...
set xelab_cmd [list xelab --debug off --relax --mt 8 -L work -L unisims_ver -L unimacro_ver -L secureip -s tb_hestia_faithful_behav work.tb_hestia_faithful work.glbl]
...
set xsim_cmd [list xsim tb_hestia_faithful_behav -wdb /dev/null -tclbatch [file join $build_root xsim_run.tcl] -log simulate.log]
...
close $fh
if {[string first "PASS: Hestia faithful variable-cell markdown regression passed" $sim_text] < 0 ||
    [string first "Fatal:" $sim_text] >= 0} {
  puts "ERROR: faithful simulation did not reach a clean PASS marker"
  cd $old_dir
  exit 1
}
cd $old_dir
