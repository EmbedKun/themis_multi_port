...
set csv_file [expr {[info exists ::env(HESTIA_ILA_CSV)] && $::env(HESTIA_ILA_CSV) ne "" ? [file normalize $::env(HESTIA_ILA_CSV)] : [file join $build_root hw_ila_capture.csv]}]
set wdb_file [expr {[info exists ::env(HESTIA_ILA_WDB)] && $::env(HESTIA_ILA_WDB) ne "" ? [file normalize $::env(HESTIA_ILA_WDB)] : [file join $build_root hw_ila_capture.wdb]}]
...
foreach probe [get_hw_probes *mp_top_done -of_objects $ila] {
  set done_probe $probe
  break
}
if {$done_probe eq ""} {
  foreach probe [get_hw_probes *probe1* -of_objects $ila] {
    set done_probe $probe
    break
  }
}
if {$done_probe eq ""} {
  foreach probe [get_hw_probes *done* -of_objects $ila] {
    if {![string match *calib_done* $probe]} {
      set done_probe $probe
      break
    }
  }
...
if {$done_probe eq ""} {
  puts "ERROR: done/probe0 trigger probe not found"
  exit 1
}
set_property TRIGGER_COMPARE_VALUE "eq1'b1" $done_probe
catch {set_property CONTROL.TRIGGER_POSITION 0 $ila}

puts "Hestia ILA probe: $done_probe"
puts "Hestia ILA trigger: done == 1"
...
puts "Full probe0 dbg_bus low bits: [31:0]=generated [63:32]=dequeued [95:64]=sram_dequeue [127:96]=direct_ddr_dequeue [159:128]=rank_errors."
puts "SRAM dequeue hit rate = sram_dequeue / dequeued; rank correctness is rank_errors == 0."
exit 0
