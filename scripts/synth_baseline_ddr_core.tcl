set script_dir [file dirname [file normalize [info script]]]
set repo_dir [file normalize [file join $script_dir ..]]

proc hestia_env_or {name default_value} {
  if {[info exists ::env($name)] && $::env($name) ne ""} {
    return $::env($name)
  }
  return $default_value
}

proc hestia_parse_util_metric {report_path label} {
  if {![file exists $report_path]} {
    return -1
  }
  set fh [open $report_path r]
  set value -1
  set pattern "\\|\\s*$label\\*?\\s*\\|\\s*([0-9.]+)\\s*\\|"
  while {[gets $fh line] >= 0} {
    if {[regexp $pattern $line -> raw_value]} {
      set value [expr {int(double($raw_value))}]
      break
    }
  }
  close $fh
  return $value
}

proc hestia_parse_wns {report_path} {
  if {![file exists $report_path]} {
    return ""
  }
  set fh [open $report_path r]
  set in_summary 0
  set value ""
  while {[gets $fh line] >= 0} {
    if {[string first "Design Timing Summary" $line] >= 0} {
      set in_summary 1
      continue
    }
    if {$in_summary && [regexp {^\s*([-+]?[0-9]+\.[0-9]+)\s+[-+]?[0-9]+\.[0-9]+\s+\d+} $line -> raw_value]} {
      set value $raw_value
      break
    }
  }
  close $fh
  return $value
}

proc hestia_policy_name {policy} {
  switch -- $policy {
    0 { return "dt_ddr" }
    1 { return "occamy_head_ddr" }
    2 { return "occamy_max_ddr" }
    3 { return "obm_ddr" }
    4 { return "hybrid_themis_ddr" }
    default { return "unknown" }
  }
}

set policy_mode [hestia_env_or HESTIA_BASELINE_DDR_POLICY_MODE 0]
set policy_name [hestia_policy_name $policy_mode]
set policy_alpha_shift [hestia_env_or HESTIA_BASELINE_DDR_POLICY_ALPHA_SHIFT 0]
set external_metadata [hestia_env_or HESTIA_BASELINE_DDR_EXTERNAL_METADATA 1]
set part_name [hestia_env_or HESTIA_BASELINE_DDR_PART xcu200-fsgd2104-2-e]
set clk_period_ns [hestia_env_or HESTIA_BASELINE_DDR_CLK_PERIOD_NS 3.333]

if {[llength $argv] > 0} {
  set build_root [file normalize [lindex $argv 0]]
} elseif {[info exists ::env(HESTIA_BASELINE_DDR_SYNTH_ROOT)] && $::env(HESTIA_BASELINE_DDR_SYNTH_ROOT) ne ""} {
  set build_root [file normalize $::env(HESTIA_BASELINE_DDR_SYNTH_ROOT)]
} else {
  set build_root [file join $repo_dir build hestia_baseline_${policy_name}_4p]
}
if {[llength $argv] > 1} {
  set clk_period_ns [lindex $argv 1]
}
file mkdir $build_root

set ports [hestia_env_or HESTIA_BASELINE_DDR_PORTS 4]
set rank_width [hestia_env_or HESTIA_BASELINE_DDR_RANK_WIDTH 6]
set seq_width [hestia_env_or HESTIA_BASELINE_DDR_SEQ_WIDTH 16]
set payload_width [hestia_env_or HESTIA_BASELINE_DDR_PAYLOAD_WIDTH 32]
set cell_count_width [hestia_env_or HESTIA_BASELINE_DDR_CELL_COUNT_WIDTH 4]
set axi_addr_width [hestia_env_or HESTIA_BASELINE_DDR_AXI_ADDR_WIDTH 64]
set axi_data_width [hestia_env_or HESTIA_BASELINE_DDR_AXI_DATA_WIDTH 512]
set axi_id_width [hestia_env_or HESTIA_BASELINE_DDR_AXI_ID_WIDTH 4]
set sram_cells [hestia_env_or HESTIA_BASELINE_DDR_SRAM_CELLS 32]
set batch_size [hestia_env_or HESTIA_BASELINE_DDR_BATCH_SIZE 8]
set batch_slots [hestia_env_or HESTIA_BASELINE_DDR_BATCH_SLOTS 32]
set packet_slots [hestia_env_or HESTIA_BASELINE_DDR_PACKET_SLOTS 64]
set bbq_bitmap_width [hestia_env_or HESTIA_BASELINE_DDR_BBQ_BITMAP_WIDTH 8]

set rtl_files [list \
  [file join $repo_dir rtl hestia_pkg.sv] \
  [file join $repo_dir rtl hestia_port_bbq.sv] \
  [file join $repo_dir rtl hestia_policy_dt.sv] \
  [file join $repo_dir rtl hestia_policy_occamy.sv] \
  [file join $repo_dir rtl hestia_policy_obm.sv] \
  [file join $repo_dir rtl hestia_policy_hybrid_themis.sv] \
  [file join $repo_dir rtl hestia_paper_scale_resource_core.sv] \
  [file join $repo_dir rtl hestia_core_ddr_bbq_extmeta.sv] \
  [file join $repo_dir rtl hestia_core_ddr_bbq.sv] \
  [file join $repo_dir rtl hestia_resource_core.sv] \
]

read_verilog -sv $rtl_files
synth_design -top hestia_resource_core -part $part_name -mode out_of_context \
  -generic PORTS=$ports \
  -generic RANK_WIDTH=$rank_width \
  -generic SEQ_WIDTH=$seq_width \
  -generic PAYLOAD_WIDTH=$payload_width \
  -generic CELL_COUNT_WIDTH=$cell_count_width \
  -generic AXI_ADDR_WIDTH=$axi_addr_width \
  -generic AXI_DATA_WIDTH=$axi_data_width \
  -generic AXI_ID_WIDTH=$axi_id_width \
  -generic SRAM_CELLS=$sram_cells \
  -generic BATCH_SIZE=$batch_size \
  -generic BATCH_SLOTS=$batch_slots \
  -generic PACKET_SLOTS=$packet_slots \
  -generic BBQ_BITMAP_WIDTH=$bbq_bitmap_width \
  -generic POLICY_MODE=$policy_mode \
  -generic POLICY_ALPHA_SHIFT=$policy_alpha_shift \
  -generic EXTERNAL_METADATA=$external_metadata \
  -generic ENABLE_DDR_META_CHECK=0
create_clock -period $clk_period_ns -name clk [get_ports clk]

set util_report [file join $build_root baseline_ddr_utilization_synth.rpt]
set hier_report [file join $build_root baseline_ddr_utilization_hier_synth.rpt]
set timing_report [file join $build_root baseline_ddr_timing_summary_synth.rpt]
report_utilization -file $util_report
report_utilization -hierarchical -file $hier_report
report_timing_summary -max_paths 10 -file $timing_report

set lut [hestia_parse_util_metric $util_report "CLB LUTs"]
set ff [hestia_parse_util_metric $util_report "CLB Registers"]
set lutram [hestia_parse_util_metric $util_report "LUT as Memory"]
set bram [hestia_parse_util_metric $util_report "Block RAM Tile"]
set uram [hestia_parse_util_metric $util_report "URAM"]
set dsp [hestia_parse_util_metric $util_report "DSPs"]
set wns [hestia_parse_wns $timing_report]

set csv_path [file join $build_root baseline_ddr_resource.csv]
set csv [open $csv_path w]
puts $csv "policy_mode,policy_name,ports,rank_width,sram_cells,batch_size,batch_slots,packet_slots,bbq_bitmap_width,policy_alpha_shift,external_metadata,clb_luts,clb_registers,lutram,bram_tiles,uram,dsps,wns_ns"
puts $csv "$policy_mode,$policy_name,$ports,$rank_width,$sram_cells,$batch_size,$batch_slots,$packet_slots,$bbq_bitmap_width,$policy_alpha_shift,$external_metadata,$lut,$ff,$lutram,$bram,$uram,$dsp,$wns"
close $csv

puts "Hestia DDR-aware baseline utilization: $util_report"
puts "Hestia DDR-aware baseline hierarchical utilization: $hier_report"
puts "Hestia DDR-aware baseline timing summary: $timing_report"
puts "Hestia DDR-aware baseline policy=$policy_mode ($policy_name) external_metadata=$external_metadata: LUT=$lut FF=$ff LUTRAM=$lutram BRAM=$bram URAM=$uram DSP=$dsp WNS=$wns"
exit 0
