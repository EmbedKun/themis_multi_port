...
} elseif {[info exists ::env(HESTIA_HW_TOP_BUILD_ROOT)] && $::env(HESTIA_HW_TOP_BUILD_ROOT) ne ""} {
  set build_root [file normalize $::env(HESTIA_HW_TOP_BUILD_ROOT)]
} else {
  set build_root [file join $repo_dir build hestia_hw_top_sim]
}
...
set sim_files [list [file join $repo_dir sim tb_hestia_hw_top.sv]]
...
set xelab_cmd [list xelab --debug off --relax --mt 8 -L work -L unisims_ver -L unimacro_ver -L secureip -s tb_hestia_hw_top_behav work.tb_hestia_hw_top work.glbl]
...
set xsim_cmd [list xsim tb_hestia_hw_top_behav -wdb /dev/null -tclbatch [file join $build_root xsim_run.tcl] -log simulate.log]
...
close $fh
if {[string first "PASS: Hestia hardware generator/checker wrapper completed" $sim_text] < 0 ||
    [string first "Fatal:" $sim_text] >= 0} {
  puts "ERROR: hardware-top simulation did not reach a clean PASS marker"
  cd $old_dir
  exit 1
}
cd $old_dir
