...
set clk_period_ns [hestia_env_or HESTIA_BASELINE_CLK_PERIOD_NS 3.333]
if {[llength $argv] > 0} {
  set build_root [file normalize [lindex $argv 0]]
} elseif {[info exists ::env(HESTIA_BASELINE_SYNTH_ROOT)] && $::env(HESTIA_BASELINE_SYNTH_ROOT) ne ""} {
  set build_root [file normalize $::env(HESTIA_BASELINE_SYNTH_ROOT)]
} else {
...
set util_report [file join $build_root baseline_shared_utilization_synth.rpt]
set hier_report [file join $build_root baseline_shared_utilization_hier_synth.rpt]
set timing_report [file join $build_root baseline_shared_timing_summary_synth.rpt]
...
set md_path [file join $build_root baseline_shared_resource.md]
set md [open $md_path w]
puts $md "# Hestia Baseline Shared-Core Resource"
puts $md ""
puts $md "| Field | Value |"
puts $md "| --- | ---: |"
...
set csv_path [file join $build_root baseline_shared_resource.csv]
set csv [open $csv_path w]
puts $csv "policy_mode,policy_name,ports,rank_width,sram_cells,packet_slots,bbq_bitmap_width,clb_luts,clb_registers,lutram,bram_tiles,uram,dsps,wns_ns"
puts $csv "$policy_mode,$policy_name,$synth_ports,$synth_rank_width,$synth_sram_cells,$synth_packet_slots,$synth_bbq_bitmap_width,$lut,$ff,$lutram,$bram,$uram,$dsp,$wns"
close $csv

puts "Hestia baseline shared-core utilization: $util_report"
puts "Hestia baseline shared-core hierarchical utilization: $hier_report"
puts "Hestia baseline shared-core timing summary: $timing_report"
puts "Hestia baseline policy=$policy_mode ($policy_name) raw resource-core: LUT=$lut FF=$ff LUTRAM=$lutram BRAM=$bram URAM=$uram DSP=$dsp WNS=$wns"
exit 0
