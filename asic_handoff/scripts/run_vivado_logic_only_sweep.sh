...
for idx in "${!policy_keys[@]}"; do
  ...
  for p in ${ports_list}; do
    ...
    (
      ...
      export HESTIA_ASIC_ENABLE_OBSERVABILITY="${HESTIA_ASIC_ENABLE_OBSERVABILITY:-0}"
      export HESTIA_ASIC_PIPELINE_SCHEDULER_CANDIDATES="${HESTIA_ASIC_PIPELINE_SCHEDULER_CANDIDATES:-1}"
      export HESTIA_ASIC_PIPELINE_PORT_QUEUE_INPUTS="${HESTIA_ASIC_PIPELINE_PORT_QUEUE_INPUTS:-0}"
