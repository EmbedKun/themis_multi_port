...
$managedEnv = @(
  ...
  "HESTIA_ASIC_ENABLE_OBSERVABILITY",
  "HESTIA_ASIC_PIPELINE_SCHEDULER_CANDIDATES",
  "HESTIA_ASIC_PIPELINE_PORT_QUEUE_INPUTS"
)
...
foreach ($policy in $policies) {
  ...
  foreach ($p in $PortsList) {
    ...
    Set-HestiaAsicEnv @{
      ...
      HESTIA_ASIC_ENABLE_OBSERVABILITY = 0
      HESTIA_ASIC_PIPELINE_SCHEDULER_CANDIDATES = 1
      HESTIA_ASIC_PIPELINE_PORT_QUEUE_INPUTS = 0
    }
